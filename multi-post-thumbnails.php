...
Version 1.7.0 (WordPress 4.5+ bugfix) by Dumitru Brinzan (http://www.brinzan.com/)
*/
...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (is_array($this->post_type)) {
				foreach ($this->post_type as $post_type) {
					add_action("wp_ajax_set-{$post_type}-{$this->id}-thumbnail", array($this, 'set_thumbnail'));
				}
			} else {
		...
		public function get_meta_key() {
			...
			return "{$post_type}_{$this->id}_thumbnail_id";
		}
		...
		public function add_metabox() {
			global $post_ID;
			$post_type = get_post_type($post_ID);
			add_meta_box("{$post_type}-{$this->id}", __($this->label, 'multiple-post-thumbnails'), array($this, 'thumbnail_meta_box'), $post_type, $this->context, $this->priority);
		}
		...
		public static function get_post_thumbnail_id($post_type, $id, $post_id) {
			$post_type_new = get_post_type($post_id);
			return get_post_meta($post_id, "{$post_type_new}_{$id}_thumbnail_id", true);
		}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$url_class = "";
			$ajax_nonce = wp_create_nonce("set_post_thumbnail-{$post_type}-{$this->id}-{$post_ID}");
			...
			if (version_compare($wp_version, '3.5', '<')) {
				...
				$modal_js = sprintf(
					...
					$post_type, $this->id, md5($this->id), $ajax_nonce
				);
			}
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_attr__( "Set %s" , 'multiple-post-thumbnails' ), $this->label ), esc_url($image_library_url), $post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if ( !isset($_wp_additional_image_sizes["{$post_type}-{$this->id}-thumbnail"])) {
						$thumbnail_html = wp_get_attachment_image( $thumbnail_id, array($content_width, $content_width), false, $attr );
				} else {
						$thumbnail_html = wp_get_attachment_image( $thumbnail_id, "{$post_type}-{$this->id}-thumbnail", false, $attr );
				}
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf( $format_string, $post_type, $this->id, sprintf( esc_html__( "Remove %s", 'multiple-post-thumbnails' ), $this->label ), $ajax_nonce );
				}
			...
			return apply_filters( sprintf( '%s_%s_admin_post_thumbnail_html', $post_type, $this->id ), $content, $post_ID, $thumbnail_id );
		}
		...
		public function set_thumbnail() {
			...
			check_ajax_referer("set_post_thumbnail-{$post_type}-{$this->id}-{$post_ID}");
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					$this->set_meta($post_ID, $post_type, $this->id, $thumbnail_id);
