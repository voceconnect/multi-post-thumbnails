...
class MultiPostThumbnails {
	...
	function __construct( $args, $attach_hooks = true ) {

		if ( $this->parse_arguments( $args ) ) {
	...
	/**
	 * Validate arguments and set up instance variables
	 *
	 * Required $args:
	...
	function parse_arguments( $args ) {
		...
		if ( is_null( $this->label ) || is_null( $this->id ) ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				$error = __( "The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'multiple-post-thumbnails' );

				trigger_error( sprintf( $error, __CLASS__, __FUNCTION__ ) );
	...
	/**
	 * Enable "post-thumbnails" support for the active theme
	 */
	...
	/**
	 * Attach all necessary hooks for the plugin
	 *
	...
	function thumbnail_meta_box() {
		...
		$thumbnail_id = $this->get_thumbnail_id( $post->ID );
	...
	/**
	 * Add thumbnail set link to pre-3.5 WP attachment forms
	 *
	...
	function admin_header_scripts() { // TODO: refactor as localization?
	 ...
	 * @param int $post_id
	 */
	function action_delete_attachment( $post_id ) { // TODO: no SQL?
		global $wpdb;
	 ...
	 * @return string
	 */
	function plugins_url( $relative_path, $plugin_path ) {
	 ...
	 * @return boolean
	 */
	static function has_post_thumbnail( $post_type, $id, $post_id = null ) {
		...
		return self::get_post_thumbnail_id( $post_type, $id, $post_id );

	}

	function set_thumbnail_id( $post_id, $thumbnail_id ) {

		return update_post_meta( $post_id, $this->get_meta_key(), $thumbnail_id );
	...
	static function get_post_thumbnail_id( $post_type, $id, $post_id ) {
		...
		$args = compact( 'post_type', 'id' );

		$mpt = new MultiPostThumbnails( $args, false );

		return $mpt->get_thumbnail_id( $post_id );
	 ...
	 */
	static function get_post_thumbnail_url($post_type, $id, $post_id = 0, $size = null) {
		if (!$post_id) {
			$post_id = get_the_ID();
		}
	 ...
	 */
	function set_thumbnail() { // TODO: refactor AJAX
		...
		if ($thumbnail_id && get_post($thumbnail_id)) {
			...
			if (!empty($thumbnail_html)) {
				$this->set_thumbnail_id($post_ID, $thumbnail_id);
